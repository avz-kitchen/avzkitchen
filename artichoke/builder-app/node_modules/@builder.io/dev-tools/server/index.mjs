...
`)}async function G(e,t){let r=await O(e),n=e.getFrameworks();return t.replace(/__LOCAL_APP_ID__/g,r).replace(/__DEV_TOOLS_URL__/g,e.devToolsServerUrl).replace(/__CLIENT_ID__/g,e.getClientId()).replace(/__FRAMEWORK__/g,n.length?n[0].name:"")}import{platform as Re}from"node:os";async function W(e,t){let r=new URL("/cli-auth","https://builder.io");r.searchParams.set("client_id",e.getClientId()),r.searchParams.set("host",await e.getDeviceId()),r.searchParams.set("response_type","code"),r.searchParams.set("cli","true");let n=new URL(h,e.devToolsServerUrl);n.searchParams.set(P,Re()),n.searchParams.set(A,process.versions.node);let i=e.getFrameworks();return i.length>0&&(r.searchParams.set(C,i[0].name),n.searchParams.set(C,i[0].name)),r.searchParams.set("redirect_url",n.href),r.searchParams.set("preview_url",t),r.href}async function z(e,t,r,n){try{if(!e)return g(r,n,400,"Builder context closed");let i=Ee(r);if(i.pathname===b){if(r.method==="POST"){let s=await Ae(r);if(s)try{let o=await q(e,JSON.parse(s));return Q(r,n,200,o)}catch(o){e.debug("Dev API Error",o.stack||o.message||o);let l={errors:[String(o.message||o)]};return Q(r,n,500,l)}}else if(r.method==="OPTIONS"){let s={"Access-Control-Allow-Methods":"POST, OPTIONS","Access-Control-Allow-Headers":"Content-Type, Cache-Control, X-Builder-Devtools-Version"},o=r.headers.origin;o&&o!=="null"?Y(new URL(o))&&(s["Access-Control-Allow-Origin"]=o):s["Access-Control-Allow-Origin"]="*",n.writeHead(200,s),n.end();return}return g(r,n,405,`${b} is an API endpoint used only during development by the Builder.io Devtools client-side script`)}if(i.pathname===_)return De(r,n,200,await H(e));if(i.pathname===E){let s=i.searchParams.get(v);return s?Pe(n,302,await W(e,s)):g(r,n,400,`Missing ${v} querystring`)}return i.pathname===h?we(r,n,200,await J(e)):i.pathname===c?(e.debug(`Close Dev Tools Server Requested: ${i.href}`),setTimeout(()=>{try{t.close(s=>{e.debug(`Dev Tools Server Closed Error: ${s}`)})}catch(s){e.debug(`Error closing devtools server: ${s}`)}},20),g(r,n,200,"Closing Dev Tools Server")):g(r,n,404,"Not found")}catch(i){return console.error("Dev Server Error",i),g(r,n,500,`Dev Server Error: ${i}`)}}function Q(e,t,r,n){y(e,t,r,"application/json",JSON.stringify(n))}function De(e,t,r,n){y(e,t,r,"application/javascript",n)}function we(e,t,r,n){y(e,t,r,"text/html",n)}function g(e,t,r,n){y(e,t,r,"text/plain",n)}function y(e,t,r,n,i){let s={"Content-Type":`${n}; charset=utf-8`,"Cache-Control":"no-cache, no-store, must-revalidate","X-Builder-Devtools-Version":"1.3.9"},o=e.headers.origin;o&&o!=="null"?Y(new URL(o))&&(s["Access-Control-Allow-Origin"]=o):s["Access-Control-Allow-Origin"]="*",t.writeHead(r,s),t.end(i)}function Y(e){return e.hostname==="localhost"||e.origin==="https://builder.io"||e.origin==="https://www.builder.io"||e.origin.endsWith(".builder.io")?!0:e.port===""?!1:e.port!=="80"&&e.port!=="443"}function Pe(e,t,r){e.writeHead(t,{Location:r}),e.end()}async function Ae(e){let t="";for await(let r of e)t+=r;return t}function Ee(e){let t=e.url||"/";return new URL(t,`http://${e.headers.host}`)}async function X(e){if(globalThis.__builderDevToolsServer)return e.debug("reusing existing devtools server"),globalThis.__builderDevToolsServer.setContext(e),globalThis.__builderDevToolsServer;let t=await Ie(e),r=_e((i,s)=>{z(e,r,i,s)}),n=()=>new Promise((i,s)=>{r.listening?(e.debug(`closing devtools server on port ${t}`),r.close(o=>{e=null,o?(e.debug(`error closing devtools server on port ${t}`),s(o)):(e&&e.debug(`closed devtools server on port ${t}`),i())})):(e.debug(`devtools server on port ${t} not listening`),i())});return process.once("SIGTERM",n),process.once("SIGINT",n),process.env.CI!=="true"&&process.stdin.on("end",n),globalThis.__builderDevToolsServer={url:new URL(`http://localhost:${t}`).href,port:t,setContext:i=>{e.debug(`set devtools context on server ${t}`),e=i},close:async()=>{globalThis.__builderDevToolsServer=null,await n()}},new Promise(i=>{r.listen(t,()=>{e.debug(`started devtools server on port ${t}`),i(globalThis.__builderDevToolsServer)})})}function Ie(e){return new Promise(t=>{let r=e.port;try{xe({hostname:"localhost",port:r,path:c,method:"GET"},i=>{i.on("end",()=>{e.debug(`${c} - Response ended`),t(r)})}).on("error",i=>{e.debug(`${c} - No response ${i}`),t(r)}).end()}catch(n){e.debug(`${c} - Error ${n}`),t(r)}})}async function Oe(e){let t={getPastSyncInfo:()=>null,resyncSnippet:()=>null,...e,devToolsServerUrl:"",isValid:!1,publicApiKey:"",serverShouldRestart:!1,port:e.port||5273,ignoreMissingConfig:!1},r=e.getFrameworks().map(s=>s.name).join(", ");t.debug(`create devtools ${Z} [${r}]`);let n=await X(t);return t.devToolsServerUrl=n.url,t.port=n.port,{getUrl:()=>t.devToolsServerUrl}}var Z="1.3.9";export{Oe as createDevToolsServer,Z as version};
